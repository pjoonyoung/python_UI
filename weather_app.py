...
form_class = uic.loadUiType('ui/weather_app.ui')[0] #ui 불러오기

class WeatherCrawler(QThread): # 쓰레드 클래스로 선언
    def __init__(self, parent):  # 초기화자
    ...
    def weather_crawling(self, weather_area):
        ...
        try:
            ...
            today_weather = weather_soup.find('span', {'class': 'weather before_slash'}).text  # 오늘 날씨(ex:맑음, 흐림, 구름많음 등등...)
            ...
            try:
                ...
                today_weather = weather_soup.find('p', {'class': 'cast_txt'}).text  # 오늘 날씨(ex:맑음, 흐림, 구름많음 등등...)
...
class WeatherApp(QMainWindow, form_class):
    def __init__(self, parent=None): #초기화자
        ...
        self.weatherInfo = WeatherCrawler(self) # 날씨 크롤러클래스의 객채를 생성
    ...
    def reflash_function(self): # 600초마다 날씨정보를 다시 호출
        # self.weather_start()
    ...
    def weather_start(self):
        ...
        if input_area == '':
            ...
            # 쓰레드클래스의 weather_crawling 메서드 호출
